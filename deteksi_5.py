...
cap = cv2.VideoCapture('30sec.mp4')
...
def infer_model(model, frame, cls_mapping):
    """
    Jalankan inferensi, mapping kelas sesuai cls_mapping.
    cls_mapping: dict {kelas_asli: kelas_baru}
    """
...
while cap.isOpened():
    ...
    # Loop pertama: visualisasi & counter
    if len(tracks) > 0:
        for t in tracks:
            x1, y1, x2, y2, tracker_id, conf, cls = t[:7]
            tracker_id = int(tracker_id)
            cls_int = int(cls)
            label = class_names[cls_int]   # 'head', 'helmet', atau 'person'
            ...
            if label == 'person':
                ...
                if cls_int == 2:    # with_helmet
                    total_person_with.add(stable_id)
                else:               # without_helmet (3)
            ...
            if label == 'head':
                ...
                if cls_int == 2:
                    color = (255, 255, 0)    # Biru? (BGR: 255,255,0 = Cyan) utk pakai helm
                else:
                    color = (0, 165, 255)    # Oranye (tanpa helm)
            ...
            # Teks
            if label == 'person':
                # Tambahkan status helm di teks? Biar ringkas: "person 001" saja.
                display_label = f"person {stable_id:03d}"
            else:
    ...
    # ========== LOGIKA PELANGGARAN (BERLAKU UNTUK head DAN person) ==========
    if len(tracks) > 0:
        for t in tracks:
            ...
            # Tentukan status helm
            if label == 'head':
                helmet_status = False   # head pasti tidak pakai helm
            else:  # person
                helmet_status = (cls_int == 2)   # 2 = with_helmet

            now = datetime.now()

            if not helmet_status:   # tidak pakai helm
                ...
                # Tampilkan peringatan sisa waktu
